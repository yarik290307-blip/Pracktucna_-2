    ...
    public class Student
    {
        ...
        public void AddLabGrade(int labNumber, byte grade)
        {
            ...
            if (labNumber >= 1 && labNumber <= 10)
            {
                ...
                _labGrades[labNumber - 1] = grade;
            }
            else
            {
                Console.WriteLine("Помилка: Номер лабораторної роботи має бути від 1 до 10.");
            }
        }
        ...
        public double GetAverageLabGrade()
        {
            ...
            for (int i = 0; i < _labGrades.Length; i++)
            {
                if (_labGrades[i] > 0)
                {
                    sum += _labGrades[i];
            ...
            return Math.Round(sum / completedLabsCount, 2);
        }
    }
}